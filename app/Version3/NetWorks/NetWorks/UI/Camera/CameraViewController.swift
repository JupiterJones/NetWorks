...
class CameraViewController: UIViewController, UIScrollViewDelegate, UIImagePickerControllerDelegate, UINavigationControllerDelegate, CLLocationManagerDelegate {
	...
	func addWatermarkToImage(image: UIImage) -> UIImage {
		...
		*/
		...
		if let currentPlacemark = placemark() {
			let postalAddress = currentPlacemark.postalAddress
			var placemarkLabel = ""
			placemarkLabel += (postalAddress?.street)! + ", "
			placemarkLabel += (postalAddress?.state)! + ", "
			placemarkLabel += (postalAddress?.postalCode)!
	...
	func newImageTitle() -> String {
		...
		//let wo = workOrderLineItem?.workOrder?.uri!
		//let fc = String(workOrderLineItem!.workOrder.files.count + 1)
		// let cn = ""
		//return j! + "-" + wo + "-" + cn + "." + fc
		//return j! + "-" + wo + "." + fc
		return "NewImageName"

...
class JobLineItemTypesController: BaseJobLineItemTypesController, UISearchBarDelegate, UISearchControllerDelegate, UISearchResultsUpdating {
	...
	func getLineItemTypes(searchString: String) {
		NWGetJobLineItemTypesService(job: job!, searchString: searchString).executeTask() {
			...
			self.jobLineItemTypes = api.jobLineItemTypes(from: response)
